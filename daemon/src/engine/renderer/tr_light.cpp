...
void R_TessLight( const trRefLight_t *light, const Color::OptionalColor& color )
{
	...
	switch( light->l.rlType )
	{
		case RL_OMNI:
		case RL_DIRECTIONAL:
			Tess_AddCube( vec3_origin, light->localBounds[ 0 ], light->localBounds[ 1 ], color.Color( Color::Named::White ) );
			break;
		case RL_PROJ:
			{
				vec3_t farCorners[ 4 ];
				...
				if ( !VectorCompare( light->l.projStart, vec3_origin ) )
				{
					...
					for ( j = 0; j < 4; j++ )
					{
						...
						Tess_AddQuadStamp2( quadVerts, color.Color( Color::Named::Cyan ) );
					}
					...
					Tess_AddQuadStamp2( quadVerts, color.Color( Color::Named::Red ) );
					...
					Tess_AddQuadStamp2( quadVerts, color.Color( Color::Named::Green ) );
				}
				else
				{
					vec3_t top;
					...
					for ( j = 0; j < 4; j++ )
					{
						const Color::Color &c = color.Color( Color::Named::Cyan );
						u8vec4_t iColor;
...
void R_SetupLightFrustum( trRefLight_t *light )
{
	...
	if ( light->isStatic )
	{
		...
		R_TessLight( light, Color::DefaultColor );
