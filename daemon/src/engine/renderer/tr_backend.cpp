				   ...
				   renderDrawSurfaces_e drawSurfFilter )
{
	...
	lastSurf = backEnd.viewParms.firstDrawSurf[ toSort + 1 ];
	for ( i = backEnd.viewParms.firstDrawSurf[ fromSort ]; i < lastSurf; i++ )
	{
...
static void RB_RenderView()
{
	...
	RB_RenderDrawSurfaces( SS_DEPTH, SS_DEPTH, DRAWSURFACES_ALL );
	RB_RenderPostDepth();
			...
			tr.refdef.blurVec[2] != 0.0f )
	{
		...
		RB_RenderDrawSurfaces( SS_ENVIRONMENT_FOG, SS_OPAQUE, DRAWSURFACES_ALL_FAR );

		RB_RenderMotionBlur();

		// draw the gun and other "near" stuff
		RB_RenderDrawSurfaces( SS_ENVIRONMENT_FOG, SS_OPAQUE, DRAWSURFACES_NEAR_ENTITIES );
	}
	else
	{
		// draw everything that is opaque
		RB_RenderDrawSurfaces( SS_ENVIRONMENT_FOG, SS_OPAQUE, DRAWSURFACES_ALL );
	}
	...
	RB_RenderDrawSurfaces( SS_ENVIRONMENT_NOFOG, SS_POST_PROCESS, DRAWSURFACES_ALL );
