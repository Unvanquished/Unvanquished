		...
		if ( tr.worldHDR_RGBE )
		{
			...
			if (tr.worldDeluxeMapping) {
				...
				for (int i = 0; i < numLightmaps; i++) {
					...
					auto image = R_FindImageFile(va("%s/%s", mapName, lightmapFiles[i]), IF_NORMALMAP | IF_NOCOMPRESSION, FT_DEFAULT, WT_CLAMP, nullptr);
		...
		else
		{
			...
			for (int i = 0; i < numLightmaps; i++) {
				...
				if (!tr.worldDeluxeMapping || i % 2 == 0) {
					auto image = R_FindImageFile(va("%s/%s", mapName, lightmapFiles[i]), IF_LIGHTMAP | IF_NOCOMPRESSION, FT_LINEAR, WT_CLAMP, nullptr);
					Com_AddToGrowList(&tr.lightmaps, image);
				} else {
					auto image = R_FindImageFile(va("%s/%s", mapName, lightmapFiles[i]), IF_NORMALMAP | IF_NOCOMPRESSION, FT_LINEAR, WT_CLAMP, nullptr);
					Com_AddToGrowList(&tr.deluxemaps, image);
				}
			}
		}
