...
bool BotLoadNavMesh( const char *filename, NavData_t &nav )
{
	...
	if ( !f )
	{
		Com_Printf( "%sERROR: Cannot open Navigaton Mesh file\n", Color::NamedString::Red );
		return false;
	}

	if ( len < 0 )
	{
		Com_Printf( "%sERROR: Negative Length for Navigation Mesh file\n", Color::NamedString::Red );
		return false;
	}
	...
	{
		Com_Printf( "%sERROR: File is wrong magic\n", Color::NamedString::Red );
		FS_FCloseFile( f );
		return false;
	}
	...
	{
		Com_Printf( "%sERROR: File is wrong version found: %d want: %d\n", Color::NamedString::Red, header.version, NAVMESHSET_VERSION );
		FS_FCloseFile( f );
		return false;
	}
	...
	if ( !nav.mesh )
	{
		Com_Printf( "%sERROR: Unable to allocate nav mesh\n", Color::NamedString::Red );
		FS_FCloseFile( f );
		return false;
	}
	...
	{
		Com_Printf( "%sERROR: Could not init navmesh\n", Color::NamedString::Red );
	...
	if ( !nav.cache )
	{
		Com_Printf( "%sERROR: Could not allocate tile cache\n", Color::NamedString::Red );
		dtFreeNavMesh( nav.mesh );
		nav.mesh = nullptr;
		FS_FCloseFile( f );
		return false;
	...
	if ( dtStatusFailed( status ) )
	{
		Com_Printf( "%sERROR: Could not init tile cache\n", Color::NamedString::Red );
		dtFreeNavMesh( nav.mesh );
	...
	for ( int i = 0; i < header.numTiles; i++ )
	{
		...
		{
			Com_Printf( "%sERROR: NUll Tile in navmesh\n", Color::NamedString::Red );
			dtFreeNavMesh( nav.mesh );
			dtFreeTileCache( nav.cache );
			nav.cache = nullptr;
			nav.mesh = nullptr;
		...
		if ( !data )
		{
			Com_Printf( "%sERROR: Failed to allocate memory for tile data\n", Color::NamedString::Red );
			dtFreeNavMesh( nav.mesh );
			dtFreeTileCache( nav.cache );
			nav.cache = nullptr;
			nav.mesh = nullptr;
		...
		if ( dtStatusFailed( status ) )
		{
			Com_Printf( "%sERROR: Failed to add tile to navmesh\n", Color::NamedString::Red );
			dtFree( data );
			dtFreeTileCache( nav.cache );
			dtFreeNavMesh( nav.mesh );
			nav.cache = nullptr;
