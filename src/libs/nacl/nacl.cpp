...
Module LoadModule(const char* module, const LoaderParams* nacl_params, bool use_debugger)
{
	...
	if (nacl_params)
		...
		snprintf(root_sock_handle, sizeof(root_sock_handle), "ROOT_SOCKET=%d", PtrToInt(pair[1]));
	...
	if (nacl_params) {
		snprintf(root_sock_redir, sizeof(root_sock_redir), "%d:%d", ROOT_SOCKET_FD, PtrToInt(pair[1]));
