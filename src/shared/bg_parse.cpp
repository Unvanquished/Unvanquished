...
==============
*/
static meansOfDeath_t BG_MeansOfDeathByName( const char *name )
{
...
void BG_ParseClassAttributeFile( const char *filename, classAttributes_t *ca )
{
	...
	// check for missing mandatory fields
	{
		...
		else if ( !( defined & PRICE ) )           { token = "cost"; } // "cost" is same as "price"
		else                                       { token = nullptr; }
	...
	while ( 1 )
	{
				...
				)
		{
			...
			if ( ( defined & WIDTH ) == 0 )
			{
				...
				|| !Q_stricmp( token, "biteDmg" )
				|| !Q_stricmp( token, "clawDmg" )
				)
		{
			PARSE( text, token );
