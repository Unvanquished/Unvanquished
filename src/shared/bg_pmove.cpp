...
{
	// we are already correct, avoid wasting time doing nothing
	if ( !trace.allsolid )
	{
		return true;
	}
...
static bool PM_StepSlideMove( bool gravity, bool predictive )
{
	...
	pm->trace( &trace, start_o, pm->mins, pm->maxs, down, pm->ps->clientNum, pm->tracemask, 0, traceType_t::TT_AABB );
	...
	else
	{
		...
		pm->trace( &trace, start_o, pm->mins, pm->maxs, up, pm->ps->clientNum, pm->tracemask, 0, traceType_t::TT_AABB );
