...
Exemple usage:
	for (Parse_WordList iterator(string); *iterator; ++iterator)
	{
		printf("C str: »%s«\n", *parser);
	}
...
===============
*/
// TODO in C++20: template<std::string delimiters = ", ">
class Parse_WordList {
public:
	Parse_WordList(const std::string &);
	Parse_WordList(const char *);
	~Parse_WordList();

	const char *operator*() const;

	Parse_WordList& operator++(); // only ++i is implemented. there is no i++.
