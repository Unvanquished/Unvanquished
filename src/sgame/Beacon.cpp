...
namespace Beacon //this should eventually become a class
{
	static int RemoveSimilar( glm::vec3 const& origin, beaconType_t type, int data, int team, int owner,
	                          int radius = 128.0f, int eFlags = 0, int eFlagsRelevant = 0 );
	...
	glm::vec3 MoveTowardsRoom( glm::vec3 const& origin )
	{
		...
		for ( glm::vec3 const& v : vecs )
		{
			...
			trace_t tr;
			trap_Trace( &tr, origin, glm::vec3(), glm::vec3(), end, 0, MASK_SOLID, 0 );
	                               ...
	                               float radius, int eFlags, int eFlagsRelevant )
	{
		...
		for ( gentity_t *ent = nullptr; (ent = G_IterateEntities(ent)); )
		{
			...
			else
			{
				...
				if ( !trap_InPVS( ent->s.origin, GLM4READ( origin ) ) )
					continue;
			}
	 ...
	 */
	static int RemoveSimilar( glm::vec3 const& origin, beaconType_t type, int data, int team, int owner,
	                          int radius, int eFlags, int eFlagsRelevant )
	{
		gentity_t *ent;
		int numRemoved = 0;
	...
	gentity_t *TagTrace( glm::vec3 const& begin, glm::vec3 const& end, int skip, int mask, team_t team, bool refreshTagged )
	{
		...
		glm::vec3 dir = glm::normalize( end - begin );

		// Do a trace for bounding boxes under the reticle first, they are prefered
		{
			trace_t tr;
			trap_Trace( &tr, begin, {}, {}, end, skip, mask, 0 );
		...
		for( int i = 0; i < level.num_entities; i++ )
		{
			...
			if( !trap_InPVS( ent->r.currentOrigin, GLM4READ( begin ) ) )
				continue;

			// LOS
			{
				trace_t tr;
				trap_Trace( &tr, begin, {}, {}, VEC2GLM( ent->r.currentOrigin ), skip, mask, 0 );
				if( tr.entityNum != i )
					continue;
			}
	...
	void Tag( gentity_t *ent, team_t team, bool permanent )
	{
		...
		switch( ent->s.eType ) {
			...
			case entityType_t::ET_PLAYER:
				...
				dead       = Entities::IsDead( ent );
				player     = true;
				BG_BoundingBox( static_cast<class_t>( ent->client->pers.classSelection ), &mins, &maxs, nullptr, nullptr, nullptr );
