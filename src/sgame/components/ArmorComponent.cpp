...
void ArmorComponent::HandleApplyDamageModifier(float& damage, vec3_t location,
vec3_t /*direction*/, int flags, meansOfDeath_t /*meansOfDeath*/) {
	...
	VectorSubtract(location, floor, locationRelativeToFloor);
	...
	VectorSubtract(location, origin, bulletPath);
