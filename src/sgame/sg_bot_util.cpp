...
bool BotTargetInAttackRange( const gentity_t *self, botTarget_t target )
{
	...
	switch ( self->client->ps.weapon )
	{
		...
		case WP_FLAMER:
			...
			break;
		case WP_SHOTGUN:
			range = ( 50 * 8192 ) / wpa->spread; //50 is the maximum radius we want the spread to be
			secondaryRange = 0;
			break;
		case WP_MACHINEGUN:
...
glm::vec3 BotGetIdealAimLocation( gentity_t *self, const botTarget_t &target, int lagPredictTime )
{
	...
	if ( !isTargetBuildable && targetTeam == TEAM_HUMANS )
	{
		...
		// Do NOT reserve this to high skill as it makes aliens very weak
	...
	if ( !isTargetBuildable && self->botMind->botSkill.level >= 5 )
	{
...
void BotFireWeaponAI( gentity_t *self )
{
	...
	switch ( self->s.weapon )
	{
		...
		case WP_ALEVEL3_UPG:
		{
			...
			constexpr bool SAFE_BARBS = true;
			bool barbIsSafe = distance > (barbSafetyFactor * BG_Missile(MIS_BOUNCEBALL)->splashRadius) || !SAFE_BARBS;
		...
		case WP_LUCIFER_CANNON:
			if ( ps.weaponCharge < LCANNON_CHARGE_TIME_MAX * Math::Clamp( random(), 0.5f, 1.0f ) )
			{
				...
				{
					if ( self->botMind->botSkill.level >= 5 && botTarget->client && level.time - botTarget->client->lastCombatTime < 4000 )
					{
