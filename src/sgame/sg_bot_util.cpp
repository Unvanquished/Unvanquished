...
void BotAimAtEnemy( gentity_t *self )
{
	...
	if ( BG_GetPlayerWeapon( &self->client->ps ) == WP_CHAINGUN && self->client->ps.weaponTime > 0 )
	{
		...
		Log::Notice("timeDelta=%d weaponTime=%d buttonATK=%s angleUpdate=%f",
			timeDelta, self->client->ps.weaponTime, usercmdButtonPressed(self->botMind->cmdBuffer.buttons, BTN_ATTACK), angleUpdate );
	}
}
