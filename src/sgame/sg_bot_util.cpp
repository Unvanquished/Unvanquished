...
botTarget_t BotGetRoamTarget( const gentity_t *self )
{
	...
	if ( !BotFindRandomPointInRadius( botClientNum, VEC2GLM( g_entities[ botClientNum ].s.origin ), point, 2000 ) )
	{
...
bool BotTargetIsVisible( const gentity_t *self, botTarget_t target, int mask )
{
	...
	trap_Trace( &trace, &muzzle[0], nullptr, nullptr, &targetPos[0], self->s.number, mask,
...
bool BotEvolveToClass( gentity_t *ent, class_t newClass )
{
	...
	if ( ent->client->pers.classSelection != PCL_NONE )
	{
		...
		if ( G_RoomForClassChange( ent, newClass, &infestOrigin[0] ) && BotCanEvolveToClass( ent, newClass ) )
		{
