...
#define Maybe_TriggerMenu(num, reason) do { if ( !quiet ) G_TriggerMenu( (num), (reason) ); } while ( 0 )
	...
	if ( weapon != WP_NONE )
	{
		...
		{
			//can afford this?
			if ( x_freeUpgrades.integer != 1 && BG_Weapon( weapon )->price > ( short ) ent->client->pers.credit )
			{
	...
	else if ( upgrade != UP_NONE )
	{
		...
		{
			//can afford this?
			if ( x_freeUpgrades.integer != 1 && BG_Upgrade( upgrade )->price > ( short ) ent->client->pers.credit )
			{
				// if requested, try to sell then recheck
