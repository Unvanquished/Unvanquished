...
static int MapSingleLuxel(rawLightmap_t * lm, surfaceInfo_t * info, bspDrawVert_t * dv, vec4_t plane, float pass, vec3_t stv[3],
						  vec3_t ttv[3], vec3_t worldverts[3])
{
	...
	if(worldverts != NULL && lightmapTriangleCheck)
	{
		...
		PlaneFromPoints(hostplane, cverts[0], cverts[1], cverts[2], qtrue);
		...
		for(j = 0; j < 3; j++)
		{
			for(i = 0; i < 3; i++)
			{
				...
				PlaneFromPoints(sideplane, cverts[i], cverts[next], temp, qtrue);
...
static void MapTriangle_r(rawLightmap_t * lm, surfaceInfo_t * info, bspDrawVert_t * dv[3], vec4_t plane, vec3_t stv[3],
						  vec3_t ttv[3], vec3_t worldverts[3])
{
...
static qboolean MapTriangle(rawLightmap_t * lm, surfaceInfo_t * info, bspDrawVert_t * dv[3], qboolean mapNonAxial)
{
	...
	else if(PlaneFromPoints(plane, dv[0]->xyz, dv[1]->xyz, dv[2]->xyz, qtrue) == qfalse)
		return qfalse;
...
void MapRawLightmap(int rawLightmapNum)
{
			...
			case MST_PATCH:
				...
				if(lm->plane)
				{
					Sys_Printf("Planar patch: [%1.3f %1.3f %1.3f] [%1.3f %1.3f %1.3f] [%1.3f %1.3f %1.3f]\n",
							   ...
							   lm->vecs[0][0], lm->vecs[0][1], lm->vecs[0][2], lm->vecs[1][0], lm->vecs[1][1], lm->vecs[1][2]);
				}
#endif
	...
	for(y = 0; y < lm->sh; y++)
	{
		for(x = 0; x < lm->sw; x++)
		{
			...
			/* report bogus origin */
			Sys_Printf
				("%6d [%2d,%2d] (%4d): XYZ(%+4.1f %+4.1f %+4.1f) LO(%+4.1f %+4.1f %+4.1f) HI(%+4.1f %+4.1f %+4.1f) <%3.0f>\n",
				 rawLightmapNum, x, y, *cluster, origin[0], origin[1], origin[2], mins[0], mins[1], mins[2], maxs[0], maxs[1],
				 maxs[2], luxel[3]);
		}
	}
#endif
...
static qboolean SubmapRawLuxel(rawLightmap_t * lm, int x, int y, float bx, float by, int *sampleCluster, vec3_t sampleOrigin,
							   vec3_t sampleNormal)
{
	...
	*sampleCluster = ClusterForPointExtFilter(sampleOrigin, (LUXEL_EPSILON * 2), lm->numLightClusters, lm->lightClusters);
	...
	//% if( VectorNormalize2( sampleNormal, sampleNormal ) <= 0.0f )
	//%     return qfalse;
	normal = SUPER_NORMAL(x, y);
...
static void SubsampleRawLuxel_r(rawLightmap_t * lm, trace_t * trace, vec3_t sampleOrigin, int x, int y, float bias,
								float *lightLuxel, float *lightDeluxel)
{
	...
	if((lightLuxel[3] + 1.0f) < lightSamples &&
	   (total[0] > 4.0f || total[1] > 4.0f || total[2] > 4.0f) && lighted != 0 && lighted != mapped)
	{
		for(b = 0; b < 4; b++)
		{
			if(cluster[b] < 0)
...
static void RandomSubsampleRawLuxel(rawLightmap_t * lm, trace_t * trace, vec3_t sampleOrigin, int x, int y, float bias,
									float *lightLuxel, float *lightDeluxel)
{
	int             b, mapped;
	int             cluster;
						...
						if(lighted != 0 && lighted != mapped)
						{
							for(t = 0; t < 4; t++)
							{
								...
								if(lightRandomSamples)
									RandomSubsampleRawLuxel(lm, &trace, origin, sx, sy, 0.5f * lightSamplesSearchBoxSize,
															lightLuxel, deluxemap ? lightDeluxel : NULL);
								else
									SubsampleRawLuxel_r(lm, &trace, origin, sx, sy, 0.25f * lightSamplesSearchBoxSize, lightLuxel,
														deluxemap ? lightDeluxel : NULL);
	...
	if(lm == NULL || cpmaHack)
	{
		...
		for(i = 0; i < ds->numVerts; i++)
		{
			...
			else
			{
				...
				trace.cluster =
					ClusterForPointExtFilter(verts[i].xyz, VERTEX_EPSILON, info->numSurfaceClusters,
											 &surfaceClusters[info->firstSurfaceCluster]);
				if(trace.cluster >= 0)
				{
					/* setup trace */
					...
					for(x = 0; x < 5; x++)
					{
						...
						for(y = 0; y < 5; y++)
						{
							...
							for(z = 0; z < 5; z++)
							{
								...
								trace.cluster =
									ClusterForPointExtFilter(origin, VERTEX_EPSILON, info->numSurfaceClusters,
															 &surfaceClusters[info->firstSurfaceCluster]);
								if(trace.cluster < 0)
									continue;
								...
								{
									floodLightAmount =
										floodlightIntensity * FloodLightForSample(&trace, floodlightDistance,
								...
								if(radVertLuxel[0] > ambientColor[0] ||
								   radVertLuxel[1] > ambientColor[1] || radVertLuxel[2] > ambientColor[2])
									x = y = z = 1000;
							}
						}
					}
...
int ShaderForPointInLeaf(vec3_t point, int leafNum, float epsilon, int wantContentFlags, int wantSurfaceFlags, int *contentFlags,
						 int *surfaceFlags)
{
	int             i, j;
	float           dot;
...
void CreateTraceLightsForSurface(int num, trace_t * trace)
{
	...
	CreateTraceLightsForBounds(mins, maxs, normal, info->numSurfaceClusters, &surfaceClusters[info->firstSurfaceCluster],
							   LIGHT_SURFACES, trace);
}
...
void FloodLightRawLightmapPass(rawLightmap_t * lm, vec3_t lmFloodLightRGB, float lmFloodLightIntensity,
							   float lmFloodLightDistance, qboolean lmFloodLightLowQuality, float floodlightDirectionScale)
{
	int             i, x, y, *cluster;
...
void FloodLightRawLightmap(int rawLightmapNum)
{
	...
	if(floodlighty && floodlightIntensity)
		FloodLightRawLightmapPass(lm, floodlightRGB, floodlightIntensity, floodlightDistance, floodlight_lowquality,
								  floodlightDirectionScale);

	/* custom pass */
	if(lm->floodlightIntensity)
	{
		FloodLightRawLightmapPass(lm, lm->floodlightRGB, lm->floodlightIntensity, lm->floodlightDistance, qfalse,
								  lm->floodlightDirectionScale);
