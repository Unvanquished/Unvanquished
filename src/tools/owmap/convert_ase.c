...
/*
ConvertLight()
exports a bsp light to an ase chunk
*/
...
*LIGHTOBJECT {
	...
	*NODE_TM {
		...
		*TM_ROW3 -0.0527	-1.2494	2.6609
		*TM_POS -0.0527	-1.2494	2.6609
...
static void ConvertLight(FILE * f, entity_t * e, int lightNum)
{
	...
	if(value[0] == '\0')
		...
		GetVectorForKey(e, "_color", color);

	/* print object header */
	sprintf(name, "Omni%d", lightNum);
	fprintf(f, "*LIGHTOBJECT\t{\r\n");
	fprintf(f, "\t*NODE_NAME\t\"%s\"\r\n", name);
	fprintf(f, "\t*NODE_TM\t{\r\n");
	fprintf(f, "\t\t*NODE_NAME\t\"%s\"\r\n", name);
	fprintf(f, "\t\t*INHERIT_POS\t0\t0\t0\r\n");
	...
	fprintf(f, "\t\t*TM_ROW2\t0\t0\t1.0\r\n");
	fprintf(f, "\t\t*TM_ROW3\t0\t0\t0\r\n");
	fprintf(f, "\t\t*TM_SCALE\t1.0\t1.0\t1.0\r\n");
	fprintf(f, "\t\t*TM_SCALEAXIS\t0\t0\t0\r\n");
	fprintf(f, "\t\t*TM_SCALEAXISANG\t0\r\n");
	fprintf(f, "\t\t*TM_POS\t%f\t%f\t%f\r\n", origin[0], origin[1], origin[2]);
	fprintf(f, "\t}\r\n");

	/* print misc settings */
	fprintf(f, "\t*LIGHT_SHADOWS\tMapped\r\n");
	fprintf(f, "\t*LIGHT_USELIGHT\t1\r\n");
	fprintf(f, "\t*LIGHT_SPOTSHAPE\tCircle\r\n");
	fprintf(f, "\t*LIGHT_USEGLOBAL\t0\r\n");
	fprintf(f, "\t*LIGHT_ABSMAPBIAS\t0\r\n");
	fprintf(f, "\t*LIGHT_OVERSHOOT\t0\r\n");

	/* print light settings */
	fprintf(f, "\t*LIGHT_SETTINGS\t{\r\n");
	fprintf(f, "\t\t*TIMEVALUE\t0\r\n");
	fprintf(f, "\t\t*LIGHT_COLOR\t%f\t%f\t%f\r\n", color[0], color[1], color[2]);
	fprintf(f, "\t\t*LIGHT_INTENS\t0.6\r\n");
	fprintf(f, "\t\t*LIGHT_ASPECT\t-1.0\r\n");
	fprintf(f, "\t\t*LIGHT_TDIST\t-1.0\r\n");
	fprintf(f, "\t\t*LIGHT_MAPBIAS\t0.19\r\n");
	fprintf(f, "\t\t*LIGHT_MAPRANGE\t4.0\r\n");
	fprintf(f, "\t\t*LIGHT_MAPSIZE\t2048\r\n");	// FIXME
	fprintf(f, "\t\t*LIGHT_RAYBIAS\t0\r\n");
	fprintf(f, "\t}\r\n");
...
int ConvertBSPToASE(char *bspName)
{
	...
	fprintf(f, "*COMMENT\t\"Generated by OWMap (OpenWolf) -convert -format ase\"\r\n");
	fprintf(f, "*SCENE\t{\r\n");
