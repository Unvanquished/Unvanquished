...
void GetFunctionNamesFromFile( char *filename, char **defines, int numdefines ) {
	source_t *source;
...
//	} //end if
	...
	for ( i = 0; i < numdefines; i++ ) {
		if ( !PC_AddDefine( source, defines[i] ) ) {
			Error( "error adding define %s", defines[i] );
			return;
		}
	}
...
void Usage() {
	Error( "USAGE: screwup <file filter> [-o <funcs> <func_desc>] [-d <define>]\n"
...
void Usage() {
	Error( "USAGE: screwup  [-o <funcs> <func_desc>] (-d <define1> (<define2> ..)) -f <file1> [<file2> ..]" );
}
