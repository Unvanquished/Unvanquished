...
itemBuildError_t G_CanBuild( gentity_t *ent, buildable_t buildable, int distance, vec3_t origin, vec3_t normal )
{
  ...
  if( ent->client->ps.stats[ STAT_TEAM ] == TEAM_ALIENS )
  {
    ...
    if( tr1.surfaceFlags & SURF_TREM_NOALIENBUILD || contents & CONTENTS_TREM_NOALIENBUILD )
  ...
  else if( ent->client->ps.stats[ STAT_TEAM ] == TEAM_HUMANS )
  {
    ...
    // Check permission to build here
    if( tr1.surfaceFlags & SURF_TREM_NOHUMANBUILD || contents & CONTENTS_TREM_NOHUMANBUILD )
      reason = IBE_PERMISSION;
  }

  // Check permission to build here
  if( tr1.surfaceFlags & SURF_TREM_NOBUILD || contents & CONTENTS_TREM_NOBUILD )
    reason = IBE_PERMISSION;
