...
void ClientImpacts( gentity_t *ent, pmove_t *pm )
{
	...
	for ( i = 0; i < pm->numtouch; i++ )
	{
		...
		if ( other->client )
		{
			...
			if((ent->client) && (other->r.svFlags & SVF_BOT) && ent->client->ps.persistant[ PERS_TEAM ] == other->client->ps.persistant[ PERS_TEAM ])
			{
				PushBot(ent, other);
			}

			// if we are standing on their head, then we should be pushed also
			if((ent->r.svFlags & SVF_BOT) && ent->s.groundEntityNum == other->s.number && other->client && ent->client->ps.persistant[ PERS_TEAM ] == other->client->ps.persistant[ PERS_TEAM ])
			{
				PushBot(other, ent);
			}
		}
