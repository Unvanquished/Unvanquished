...
{
// FIXME:STRIP - use weapon info etc.
...
void G_RewardAttackers( gentity_t *self )
{
	...
	for ( playerNum = 0; playerNum < level.maxclients; playerNum++ )
	{
		...
		else
		{
			...
			{
				player->client->pers.namelog->damageStats.kills        += share;
				player->client->pers.namelog->damageStats.killingSpree += share;
			}
...
void G_PlayerDie( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int meansOfDeath )
{
	...
	if ( attacker )
	{
		...
		else if( attacker->client && attacker->client->pers.namelog && G_OnSameTeam( self, attacker ) )
	...
	if ( attacker && attacker->client )
	{
		...
		else if ( G_OnSameTeam( self, attacker ) )
		{
			attacker->client->pers.namelog->damageStats.teamkills++;
		}
               ...
               vec3_t dir, vec3_t point, int damage, int damageFlags, int mod )
{
	...
	else if( attacker->client->pers.namelog->strip && target->s.eType == ET_PLAYER )
	{
