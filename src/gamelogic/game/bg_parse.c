...
void BG_ParseClassAttributeFile( const char *filename, classAttributes_t *ca )
{
    ...
    while ( 1 )
    {
        ...
        else if ( !Q_stricmp( token, "strippedStage" ) )
        {
            PARSE(text, token);

            ca->strippedStages = BG_StageFromNumber( atoi(token) );

            defined |= STRIPPEDSTAGE;
        }
        else
        {
            Com_Printf( S_ERROR "%s: unknown token '%s'\n", filename, token );
    ...
    if ( !( defined & STRIPPEDSTAGE ) )
    {
        ca->strippedStages = ca->stages;
    }
...
void BG_ParseWeaponAttributeFile( const char *filename, weaponAttributes_t *wa )
{
    ...
    while ( 1 )
    {
        ...
        else if ( !Q_stricmp( token, "strippedStage" ) )
        {
            ...
            wa->strippedStages = BG_StageFromNumber( atoi( token ) );
...
void BG_ParseUpgradeAttributeFile( const char *filename, upgradeAttributes_t *ua )
{
    ...
    while ( 1 )
    {
        ...
        else if ( !Q_stricmp( token, "strippedStage" ) )
        {
            ...
            ua->strippedStages = BG_StageFromNumber( atoi( token ) );
