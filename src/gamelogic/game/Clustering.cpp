...
public:
	...
	{
		for ( int i = 0; i < Dim; i++ ) if ( coords[i] != other[i] )
			{
				return false;
			}
...
public:
	...
	bool Remove( const Data& data )
	{
		...
		if ( records.find( data ) == records.end() )
		{
			return false;
		}
...
private:
	...
	void FindMST()
	{
		...
		for ( const edge_record_type & edgeRecord : edges )
		{
			...
			if ( ( mstEdges.size() + 1 ) == records.size() )
			{
				break;
			}
			...
			if ( firstVertexRepr == secondVertexRepr )
			{
				continue;
			}
...
static inline baseClusteringLayer_t GetClusteringLayer( team_t team, bool enemy )
{
	if ( team == TEAM_ALIENS )
	{
		return enemy ? BCL_ALIEN_ENEMY : BCL_ALIEN_FRIENDLY;
	}

	if ( team == TEAM_HUMANS )
	{
		return enemy ? BCL_HUMAN_ENEMY : BCL_HUMAN_FRIENDLY;
	}
...
{
	if ( layer == BCL_ALIEN_FRIENDLY || layer == BCL_ALIEN_ENEMY )
	{
		return TEAM_ALIENS;
	}

	if ( layer == BCL_HUMAN_FRIENDLY || layer == BCL_HUMAN_ENEMY )
	{
		return TEAM_HUMANS;
	}
...
{
	if ( layer == BCL_ALIEN_ENEMY || layer == BCL_HUMAN_ENEMY )
	{
		return true;
	}
