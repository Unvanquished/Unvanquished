...
void CG_mv_KeyHandling( int _key, qboolean down ) {
	...
	case K_MOUSE1:
		...
		return;
	case K_MOUSE2:
		if(!down) CG_mvSwapViews_f();	// Swap the window with the main view
		return;
	case K_INS:
	case K_KP_PGUP:
	case K_MWHEELDOWN:
		if(!down) CG_mvShowView_f();	// Make a window for the client
		return;
	case K_DEL:
	case K_KP_PGDN:
	case K_MWHEELUP:
		if(!down) CG_mvHideView_f();	// Delete the window for the client
		return;
	case K_MOUSE3:
		if(!down) CG_mvToggleView_f();	// Toggle a window for the client
		return;
