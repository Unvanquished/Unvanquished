...
//          BG_AnimParseError( "end of file without ENDANIMS" );
		...
		animations[ i ].duration = animations[ i ].initialLerp
		                           + animations[ i ].frameLerp * animations[ i ].numFrames + animations[ i ].animBlend;
	...
	if( token && token[ 0 ] )
	{
		if( animModelInfo->version < 2 || !Q_stricmp( token, "HEADFRAMES" ) )
		{
			...
			for( i = 0; i < MAX_HEAD_ANIMS; i++ )
			{
				...
				headAnims[ i ].firstFrame -=
				  animations[ MAX_ANIMATIONS - 1 ].firstFrame + animations[ MAX_ANIMATIONS - 1 ].numFrames + skip;
