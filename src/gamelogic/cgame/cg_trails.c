	...
	while ( 1 )
	{
		...
		if ( !Q_stricmp( token, "segments" ) )
		{
			...
			if ( btb->numSegments >= MAX_TRAIL_BEAM_NODES )
			{
				...
				CG_Printf( "%s", _( "^3WARNING: too many segments in trail beam\n" ));
			}
		...
		else if ( !Q_stricmp( token, "jitter" ) )
		{
			...
			{
				CG_Printf( "%s", _( "^1ERROR: too many jitters\n" ));
				break;
			}
...
static qboolean CG_ParseTrailSystem( baseTrailSystem_t *bts, char **text_p, const char *name )
{
	...
	while ( 1 )
	{
		...
		if ( !Q_stricmp( token, "{" ) )
		{
			...
			{
				CG_Printf( "%s", _( "^1ERROR: failed to parse trail beam\n" ));
				return qfalse;
			}
...
static qboolean CG_ParseTrailFile( const char *fileName )
{
	...
	while ( 1 )
	{
		...
		{
			CG_Printf( "%s", _( "^1ERROR: trail system already named\n" ));
			return qfalse;
		}
	}
...
trailSystem_t *CG_SpawnNewTrailSystem( qhandle_t psHandle )
{
	...
	if ( !bts->registered )
	{
		CG_Printf( "%s", _( "^1ERROR: a trail system has not been registered yet\n" ));
		return NULL;
	}
