...
bool Utility::AntiHumanRadiusDamage(Entity& entity, float damage, float range, meansOfDeath_t mod) {
	bool hit = false;
	...
	ForEntities<HumanClassComponent>([&] (Entity& other, HumanClassComponent& humanClassComponent) {
		...
		damage *= (1.0f - distance / range);

		if (damage <= 0.0f) return;
		if (!G_IsVisible(entity.oldEnt, other.oldEnt, MASK_SOLID)) return;

		other.Damage(damage, entity.oldEnt, {}, {}, DAMAGE_NO_LOCDAMAGE, mod);
		hit = true;
	});
