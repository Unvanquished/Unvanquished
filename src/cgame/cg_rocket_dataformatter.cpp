...
static void CG_Rocket_DFGearOrReady( int handle, const char *data )
{
	...
	else
	{
		...
		if ( s && ( s->team == cg.predictedPlayerState.persistant[ PERS_TEAM ] || cg.predictedPlayerState.persistant[ PERS_TEAM ] == TI_NONE ) && s->weapon != WP_NONE )
		{
			rml = va( "<img src='/%s'/>", CG_GetShaderNameFromHandle( cg_weapons[ s->weapon ].weaponIcon ) );
		}

		if ( s && ( s->team == cg.predictedPlayerState.persistant[ PERS_TEAM ] || cg.predictedPlayerState.persistant[ PERS_TEAM ] == TI_NONE ) && i2t( s->team ) == TEAM_HUMANS && s->upgrade != UP_NONE )
		{
