...
static int CDataSource_Read(lua_State* L)
{
	...
	Rml::DataSource* source = Rml::DataSource::GetDataSource(sourceName);
	if (!source) {
		Log::Warn("No data source '%s'", sourceName);
		lua_pushnil(L);
		return 1;
	}
	Rml::DataQuery query(source, Rml::String(table), Rml::String(fields), 0, -1);
	lua_newtable(L);
	int row = 0;
	while ( query.NextRow() ) {
		...
		for (size_t i = 0; i < query.GetNumFields(); i++ ) {
			auto value = query.Get<Rml::String>(i, "");
